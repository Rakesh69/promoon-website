...
const Positions = () => {
  return (
    <section
      className="select-none py-10 md:my-28 scroll-mt-70px"
      id="position"
    >
      <div className="w-full py-[31px] bg-positionBgColor dark:bg-darkBorder">
        <p className="text-center font-bold text-textPrimary text-xl md:text-2xl lg:text-3xl dark:text-white">
          We are hire(Positions)
        </p>
      </div>
      <div className="max-w-screen-xl w-90% mx-auto">
        <div>
          <div className="grid grid-cols-1 grid-rows-1 md:grid-cols-1 md:grid-rows-3 gap-5 lg:gap-28 lg:py-10">
            {POSITIONS.map(
              ({ description, icon, id, positonImage, title, darkIcon }) => {
                return (
                  <div className="flex flex-col md:flex-row w-full justify-center md:justify-between items-center  gap-16 ">
                    <div className="flex flex-col justify-center md:flex-row md:items-start md:w-1/2  items-center mt-5">
                      <div className="md:pr-3 dark:hidden block">{icon}</div>
                      <div className="md:pr-3 dark:block hidden">
                        {darkIcon}
                      </div>
                      <div className="flex flex-col justify-center md:items-start items-center gap-3 md:gap-3">
                        ...
                        <p className="text-center lg:text-xl text-textSecondary md:text-start dark:text-darkTextPrimary">
                          {description}
                        </p>
